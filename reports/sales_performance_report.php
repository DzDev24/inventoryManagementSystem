...
(async () => {
  ...
  const { summary, recent_sales } = await res.json();
  ...
  const statCards = [
    { title: 'Total Orders', value: summary.total_orders, icon: 'fas fa-shopping-cart', color: '#4f46e5' },
    { title: 'Orders Completed', value: summary.orders_completed, icon: 'fas fa-check-circle', color: '#10b981' },
    { title: 'Orders Pending', value: summary.orders_pending, icon: 'fas fa-clock', color: '#f59e0b' },
    { title: 'Orders Canceled', value: summary.orders_canceled, icon: 'fas fa-times-circle', color: '#ef4444' }
  ];
