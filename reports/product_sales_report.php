...
(async () => {
  ...
  const { summary, products } = await res.json();
  ...
  const statCards = [
    { title: 'Best-Selling Product', value: summary.best_seller, icon: 'fas fa-trophy', color: '#4f46e5' },
    { title: 'Total Units Sold', value: summary.total_units.toLocaleString(), icon: 'fas fa-boxes', color: '#3b82f6' },
    { title: 'Total Product Revenue', value: 'DA ' + summary.total_revenue.toLocaleString(), icon: 'fas fa-money-bill-wave', color: '#10b981' },
    { title: 'Least-Selling Product', value: summary.least_seller, icon: 'fas fa-exclamation-triangle', color: '#f59e0b' }
  ];
