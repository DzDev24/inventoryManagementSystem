...
(async () => {
  ...
  try {
    ...
    const { summary, recent_purchases } = await res.json();
    ...
    const statCards = [
      { title: 'Total Purchases', value: summary.total_purchases, icon: 'fas fa-shopping-basket', color: '#4f46e5' },
      { title: 'Completed', value: summary.purchases_completed, icon: 'fas fa-check-circle', color: '#10b981' },
      { title: 'Pending', value: summary.purchases_pending, icon: 'fas fa-clock', color: '#f59e0b' },
      { title: 'Canceled', value: summary.purchases_canceled, icon: 'fas fa-times-circle', color: '#ef4444' },
      { title: 'Total Spent', value: summary.total_spent ? summary.total_spent.toLocaleString() + ' DA' : '0 DA', icon: 'fas fa-money-bill-wave', color: '#3b82f6' }
    ];
