...
if (!empty($customer['Media_ID'])) {
    ...
    $stmt->bind_param("i", $customer['Media_ID']);
    $stmt->execute();
...
<img id="profilePreview" class="img-account-profile rounded-circle mb-2" src="<?= htmlspecialchars($profileImage) ?>" alt="Profile Image" style="width: 160px; height: 160px; object-fit: cover;" />
        ...
        <div class="col-xl-8">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="POST" action="update_customer_profile.php" enctype="multipart/form-data">
                        <input type="hidden" name="customer_id" value="<?= $customer_id ?>">
                        
                        <div class="mb-3">
                            <label class="small mb-1" for="name">Customer Name</label>
                            <input class="form-control" id="name" name="name" type="text" required value="<?= htmlspecialchars($customer['Name']) ?>" />
                        </div>

                        <div class="mb-3">
                            <label class="small mb-1" for="email">Email</label>
                            <input class="form-control" id="email" name="email" type="email" required value="<?= htmlspecialchars($customer['Email']) ?>" />
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" id="phone" name="phone" type="text" value="<?= htmlspecialchars($customer['Phone']) ?>" />
                        </div>

                        <div class="mb-3">
                            <label class="small mb-1" for="shipping_address">Shipping Address</label>
                            <input class="form-control" id="shipping_address" name="shipping_address" type="text" value="<?= htmlspecialchars($customer['Shipping Address']) ?>" />
                        </div>
